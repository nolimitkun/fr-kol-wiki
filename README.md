...
`automation/` 提供本地日更方案：每天扫描关注频道，最多挑选 2 条高质量长视频。选题优先追踪法国热点，其次关注中法关系及法国华人华裔议题；同时保留内容质量、信息密度和可核验性门槛，不因热度降低收录标准。有字幕时直接抓取，没有字幕时调用 NVIDIA GPU 和 `faster-whisper large-v3-turbo` 转录。Codex 按本项目的白话风格翻译、归纳、交叉链接并运行完整校验，然后创建 PR，等待 GitHub Codex Review，自动核对逐字稿、修复反馈并触发复审。复审干净且 CI 通过后，任务会绑定已审核的最新提交自动合并，并确认 GitHub Pages 部署成功；没有合适候选时不产生提交。

任务使用独立的 `~/workspace/fr-kol-wiki-nightly` 克隆，不会碰正在编辑的工作区。systemd 模板默认每天巴黎时间 08:00 运行，关机或睡眠错过后会在恢复时补跑。一天内成功运行过会自动跳过重复触发。若自动审查、修复、CI 或合并中途退出，PR 会保持打开；下次运行会先接管唯一的未完成日更 PR，从原分支继续复审、合并和部署，不会另开重复 PR。若合并后的部署失败，则写入持久化阻断标记，后续日更不会继续。整个流程不会绕过检查或堆积新 PR。安装后的日志位于 `~/.local/state/fr-kol-wiki-nightly/logs/`。
