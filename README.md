...
任务使用独立的 `~/workspace/fr-kol-wiki-nightly` 克隆，不会碰正在编辑的工作区。systemd 模板默认每天巴黎时间 08:00 运行，关机或睡眠错过后会在恢复时补跑。一天内成功运行过会自动跳过重复触发。若自动审查、修复、CI 或合并失败，PR 会保持打开，任务以失败状态停止；若合并后的部署失败，则写入持久化阻断标记，后续日更不会继续。整个流程不会绕过检查或堆积新 PR。安装后的日志位于 `~/.local/state/fr-kol-wiki-nightly/logs/`。
